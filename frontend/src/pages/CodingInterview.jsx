...
export default function CodingInterview() {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6 text-slate-200 h-[calc(100vh-100px)] flex flex-col">
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              className="bg-slate-900 border border-slate-800 rounded-lg text-xs font-semibold p-2 text-white outline-none focus:border-blue-500"
            >
              {problems.map((p) => (
          ...
          <button
            ...
            disabled={executing}
            className="bg-blue-600 hover:bg-blue-500 disabled:bg-blue-800 text-white text-xs font-bold px-4 py-2 rounded-lg flex items-center gap-1.5 transition-all shadow-md"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 flex-1 min-h-0 overflow-y-auto">
        ...
        <div className="glass-panel rounded-xl border-white/5 flex flex-col min-h-0">
          ...
          <div className="p-6 overflow-y-auto flex-1 text-slate-300 leading-relaxed text-sm whitespace-pre-wrap">
            {selectedProblem ? (
              <>
                <h3 className="text-base font-extrabold text-white mb-4">{selectedProblem.title}</h3>
                <p className="text-sm text-slate-400">{selectedProblem.description}</p>
              </>
            ) : (
              <p className="text-xs italic text-slate-500">Select a problem to view descriptions.</p>
            )}
          </div>
        </div>
        ...
        <div className="flex flex-col gap-4 min-h-0 flex-1">
          {/* Custom monospaced textarea code editor */}
          <div className="glass-panel rounded-xl border-white/5 flex-1 flex flex-col min-h-0 relative">
            ...
            <textarea
              ...
              className="flex-1 bg-slate-950/80 p-4 font-mono text-sm leading-relaxed text-blue-300 outline-none resize-none overflow-y-auto w-full select-text"
              spellCheck="false"
